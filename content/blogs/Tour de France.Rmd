...
As the Tour de France 2020 is currently underway, the peloton will have to ride over 3500km in a bit less than 3 weeks. While the outcome of this year's Tour, the first one to happen in September, is quite certain, this race is usually the theatre of much suspense and drama. Every year since 1903, the Tour de France is a cycling race taking place in the summer which slowly became inherent to the french culture. Today, any citizen will closely follow the route of the Tour and hope for it to pass by its village and get the chance to see the 'real' Maillot Jaune (leader of general classification). 
...
```{r records, echo=FALSE, warning=FALSE}
# Let's display the record holders for the Tour de France

distancerecords <- tourdefrance %>%
  group_by(Year) %>% 
  summarise(fastest_pace = max(Pace, na.rm=TRUE), 
         lowest_pace = min(Pace, na.rm=TRUE), 
         longest_tour = max(Distance, na.rm=TRUE), 
         shortest_tour = min(Distance, na.rm=TRUE)
            ) %>% 
  head(1)

col_orders <- c("Fastest Pace", "Lowest Pace", "Longest Distance", "Shortest Distance")

distancerecords2 <- distancerecords[col_orders,]
distancerecords2

```

The first Tour de France was very short and consisted of only 8 stages. Rapidly, the number of stages increased and so did the distance of each Tour. Indeed the first Tours were almost 5000 km long on average, while in recent years the Tour de France was shorter (~3500km).

Concerning the speed of the riders, the professionalisation of the sport and the technological advances had the effect of dramatically increasing the speed of the peloton. The slowest Tour de France was run at an average speed of 28km/h while Lance Armstrong won one of his Tours at an average speed of over 41km/h. 
...
ggplot(data = tourdefrance) + geom_point(aes(x = Year, y = Ended/Started*100), color="steelblue") +
...
stage_details <- read.csv2(file = '../input/tour-de-france-historical-tour-2018/stages_TDF.csv', header = TRUE, sep = ',', stringsAsFactors = TRUE, encoding = 'UTF-8')
...
![](https://storage.googleapis.com/kagglesdsdata/datasets/52848/100269/France_map_plot.png?GoogleAccessId=web-data@kaggle-161607.iam.gserviceaccount.com&Expires=1537248146&Signature=Q1JBEsrMzr5fjkGJ5WQBugHDsijKkXxCf%2F%2BULBhk%2Be8MsjxA8TREi9cpdWQwHWz%2BFiotUBIC8MU6xAoj4WGUfcj9GCVYxy9yg3jRv7jNQHO7FflrtMt6l8RFG9IHwMUr%2FBztJj6zuaw0NpW8RMMGc1g0cRC0ZWVamxmd9BXzeNkgehpKMCDUWhtoc78%2FpIs6xWJcIvSOqmVWuRXXIdlbx2OcSUQTz3q3r6UGzYY98QEA%2BT4pHuGj8tliPdMwPESwFkhJK5o2NH0S6nMpnZ58k6egRMyBIPntrMs98CM6FmlZOpZ%2F0FKI2Nlf8kL6zghbjDMUmBepXXVjAfQZDyuZTA%3D%3D)

```{r stagedetails maps, echo=FALSE, warning=FALSE, message=FALSE}
library(ggmap)
library(ggalt)
...
unique_stage_geocode <- read.csv2('../input/tour-de-france-historical-tour-2018/unique_stage_geocode.csv', header = TRUE, quote = "", sep = ';', stringsAsFactors = TRUE, encoding = 'UTF-16')
...
france_ggl_map + geom_point(data = stage_loc_origin, aes(x=lon, y=lat), alpha = 0.2, size = 2, color = "tomato") +
